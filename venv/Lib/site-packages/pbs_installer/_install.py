...
def get_download_link(
    ...
    for py_ver, urls in PYTHON_VERSIONS.items():
        ...
        if not build_dir and (matched := urls.get((platform, arch, False, free_threaded))) is not None:
            return py_ver, matched
    raise ValueError(f"Could not find a version matching version={request!r}, implementation={implementation}")


def download(python_file: PythonFile, destination: StrPath, client: httpx.Client | None = None) -> str:
