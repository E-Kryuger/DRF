...
class Finder:
    ...
    def find_all(
        ...
        if allow_prereleases and (pre is False or dev is False):
            raise ValueError("If allow_prereleases is True, pre and dev must not be False.")
        if isinstance(major, str):
            if any(v is not None for v in (minor, patch, pre, dev, name)):
                raise ValueError("If major is a string, minor, patch, pre, dev and name " "must not be specified.")
    ...
    def _dedup(
        ...
        for python_version in sorted(python_versions, key=sort_key):
            ...
            if key not in result and python_version.is_valid() and version_matcher(python_version):
