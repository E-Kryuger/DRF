                ...
                try:
                    ...
                    try:
                        ...
                        if blob.id != index_sha and index_sha != committed_sha:
                            raise Error("file has staged content differing " f"from both the file and head: {p}")
...
def sparse_checkout(repo, patterns=None, force: bool = False, cone: Union[bool, None] = None):
...
def cone_mode_add(repo, dirs, force=False):
    ...
    with open_repo_closing(repo) as repo_obj:
        ...
        base_patterns = ["/*", "!/*/"]
        existing_dirs = [pat.strip("/") for pat in repo_obj.get_sparse_checkout_patterns() if pat not in base_patterns]
