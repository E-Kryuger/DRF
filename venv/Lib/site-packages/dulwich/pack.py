...
class SHA1Reader:
    ...
    def check_sha(self, allow_empty: bool = False) -> None:
        ...
        if stored != self.sha1.digest() and (
            not allow_empty or sha_to_hex(stored) != b"0000000000000000000000000000000000000000"
        ):
    ...
    def _pack_data_chunks(
        ...
        *,
        ...
        if actual_num_records != num_records:
            raise AssertionError(f"actual records written differs: {actual_num_records} != {num_records}")
...
def extend_pack(
    ...
    *,
    ...
    for i, object_id in enumerate(object_ids):
        if progress is not None:
            progress((f"writing extra base objects: {i}/{len(object_ids)}\r").encode("ascii"))
