...
class SwiftConnector:
    ...
    def swift_auth_v1(self):
        ...
        if ret.status_code < 200 or ret.status_code >= 300:
            raise SwiftException(
                "AUTH v1.0 request failed on " + f"{self.auth_url} with error code {ret.status_code} ({ret.items()!s})"
            )
    ...
    def test_root_exists(self) -> Optional[bool]:
        ...
        if ret.status_code == 404:
            return None
        if ret.status_code < 200 or ret.status_code > 300:
            raise SwiftException(f"HEAD request failed with error code {ret.status_code}")
        return True
    ...
    def get_container_objects(self):
        ...
        if ret.status_code == 404:
            return None
        if ret.status_code < 200 or ret.status_code > 300:
            raise SwiftException(f"GET request failed with error code {ret.status_code}")
        content = ret.read()
    ...
    def del_object(self, name) -> None:
        ...
        if ret.status_code < 200 or ret.status_code > 300:
            raise SwiftException(f"DELETE request failed with error code {ret.status_code}")
