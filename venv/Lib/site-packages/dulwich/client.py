...
class TraditionalGitClient(GitClient):
    ...
    def fetch_pack(
        ...
        """
        if protocol_version is not None and protocol_version not in GIT_PROTOCOL_VERSIONS:
        ...
        if server_protocol_version not in GIT_PROTOCOL_VERSIONS:
            raise ValueError(f"unknown Git protocol version {server_protocol_version} used by server")
        if protocol_version and server_protocol_version > protocol_version:
            raise ValueError(f"bad Git protocol version {server_protocol_version} used by server")
        ...
        with proto:
            ...
            if self.protocol_version == 2:
                ...
                proto.write(b"0001")  # delim-pkt
                if find_capability(negotiated_capabilities, CAPABILITY_FETCH, CAPABILITY_FILTER) and filter_spec:
...
class AbstractHttpGitClient(GitClient):
    ...
    def _discover_references(
        ...
        protocol_version: Optional[int] = None,
        ref_prefix: Optional[list[Ref]] = None,
    ) -> tuple[dict[Ref, ObjectID], set[bytes], str, dict[Ref, Ref], dict[Ref, ObjectID]]:
        if protocol_version is not None and protocol_version not in GIT_PROTOCOL_VERSIONS:
            raise ValueError(f"unknown Git protocol version {protocol_version}")
        assert base_url[-1] == "/"
        tail = "info/refs"
        ...
        try:
            ...
            if not self.dumb:
                ...
                def begin_protocol_v2(proto):
                    ...
                    body = b"".join([pkt_line(b"command=ls-refs\n"), b"0001", pkt_seq(*pkts)])

                    resp, read = self._smart_request(service.decode("ascii"), base_url, body)
                ...
                if self.protocol_version == 2:
                    ...
                    if pkt.rstrip(b"\n") != (b"# service=" + service):
                        raise GitProtocolError(f"unexpected first line {pkt!r} from smart server")
    ...
    def _smart_request(self, service, url, data):
        ...
        if resp.content_type.split(";")[0] != result_content_type:
            raise GitProtocolError(f"Invalid content-type from server: {resp.content_type}")
        return resp, read
    ...
    def send_pack(self, path, update_refs, generate_pack_data, progress=None):
        ...
        old_refs, server_capabilities, url, symrefs, peeled = self._discover_references(b"git-receive-pack", url)
        (
            negotiated_capabilities,
            agent,
        ) = self._negotiate_receive_pack_capabilities(server_capabilities)
