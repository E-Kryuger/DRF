...
class ShaFile:
    ...
    def _parse_legacy_object_header(magic, f: BinaryIO) -> "ShaFile":
        ...
        if not obj_class:
            raise ObjectFormatException("Not a known type: {}".format(type_name.decode("ascii")))
        return obj_class()
    ...
    @staticmethod
    def from_raw_string(type_num, string: bytes, sha: Optional[ObjectID] = None) -> "ShaFile":
...
def serialize_tree(items):
    ...
    for name, mode, hexsha in items:
        yield ((f"{mode:04o}").encode("ascii") + b" " + name + b"\0" + hex_to_sha(hexsha))
    ...
    def check(self) -> None:
        ...
        for name, mode, sha in parse_tree(b"".join(self._chunked_text), True):
            ...
            if b"/" in name or name in (b"", b".", b"..", b".git"):
                raise ObjectFormatException("invalid name {}".format(name.decode("utf-8", "replace")))
