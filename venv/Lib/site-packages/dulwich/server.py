...
class BackendRepo(TypingProtocol):
    ...
    def find_missing_objects(self, determine_wants, graph_walker, progress, get_tagged=None) -> Iterator[ObjectID]:
    ...
    def set_client_capabilities(self, caps: Iterable[bytes]) -> None:
        ...
        for cap in caps:
            ...
            if cap not in allowable_caps:
                raise GitProtocolError(f"Client asked for capability {cap!r} that was not advertised.")
        for cap in self.required_capabilities():
            if cap not in caps:
                raise GitProtocolError(f"Client does not support required capability {cap!r}.")
        self._client_capabilities = set(caps)
        logger.info("Client capabilities: %s", caps)

    def has_capability(self, cap: bytes) -> bool:
        if self._client_capabilities is None:
            raise GitProtocolError(f"Server attempted to access capability {cap!r} before asking client")
    ...
    def _start_pack_send_phase(self) -> None:
        if self.has_capability(CAPABILITY_SIDE_BAND_64K):
            ...
            if not self.has_capability(CAPABILITY_NO_PROGRESS):
                self.progress = partial(self.proto.write_sideband, SIDE_BAND_CHANNEL_PROGRESS)  # type: ignore
...
def serve_command(handler_cls, argv=sys.argv, backend=None, inf=sys.stdin, outf=sys.stdout) -> int:
