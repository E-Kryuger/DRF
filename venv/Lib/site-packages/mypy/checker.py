...
class TypeChecker(NodeVisitor[None], CheckerPluginInterface):
    ...
    def extract_callable_type(self, inner_type: Type | None, ctx: Context) -> CallableType | None:
        ...
        if inner_type is not None and not isinstance(inner_type, AnyType):
            ...
            if isinstance(inner_type, TypeType):
                inner_type = get_proper_type(self.expr_checker.analyze_type_type_callee(inner_type.item, ctx))
                ...
                if overload_can_never_match(sig1, sig2):
                    ...
                    with state.strict_optional_set(True):
                        if is_unsafe_overlapping_overload_signatures(sig1, sig2, type_vars):
                            flip_note = (
                                j == 0
                                and not is_unsafe_overlapping_overload_signatures(sig2, sig1, type_vars)
                                and not overload_can_never_match(sig2, sig1)
                            )
                            self.msg.overloaded_signatures_overlap(i + 1, i + j + 2, flip_note, item.func)
            ...
            with self.binder.top_frame_context():
                ...
                if typ.type_is:
                    ...
                    if ref_type is not None and (not defn.is_static or defn.name == "__new__"):
                        arg_index = 1
                    if arg_index < len(typ.arg_types) and not is_subtype(typ.type_is, typ.arg_types[arg_index]):
                        self.fail(
    ...
    def check_method_override(self, defn: FuncDef | OverloadedFuncDef | Decorator) -> list[TypeInfo] | None:
        ...
        for base in defn.info.mro[1:]:
            result = self.check_method_or_accessor_override_for_base(defn, base, check_override_compatibility)
            if result is None:
    ...
    def check_method_override_for_base_with_name(
        ...
        if base_attr:
            ...
            if isinstance(original_node, Var):
                ...
                expanded_type = expand_self_type(original_node, expanded_type, fill_typevars(defn.info))
    ...
    def check_override(
        ...
        if fail:
            ...
            if (
                ...
                for i, (sub_kind, super_kind) in enumerate(zip(override.arg_kinds, original.arg_kinds)):
    ...
    def infer_partial_type(self, name: Var, lvalue: Lvalue, init_type: Type) -> bool:
        ...
        if isinstance(init_type, NoneType):
            ...
            if (
                ...
                if isinstance(arg0, (NoneType, UninhabitedType)) and self.is_valid_defaultdict_partial_value_type(
                    arg1
                ):
    ...
    def visit_assert_stmt(self, s: AssertStmt) -> None:
        ...
        if s.msg is not None:
            self.expr_checker.analyze_cond_branch(else_map, s.msg, None, suppress_unreachable_errors=False)
    ...
    def type_check_raise(self, e: Expression, s: RaiseStmt, optional: bool = False) -> None:
        ...
        if isinstance(typ, Instance) and typ.type.fullname == "builtins._NotImplementedType":
            self.fail(
                message_registry.INVALID_EXCEPTION.with_additional_msg('; did you mean "NotImplementedError"?'),
                s,
            )
    ...
    def visit_match_stmt(self, s: MatchStmt) -> None:
        ...
        with self.binder.frame_context(can_skip=False, fall_through=0):
            ...
            for p, g, b in zip(s.patterns, s.guards, s.bodies):
                current_subject_type = self.expr_checker.narrow_type_from_binder(named_subject, subject_type)
    ...
    def _get_recursive_sub_patterns_map(self, expr: Expression, typ: Type) -> dict[Expression, Type]:
    ...
    def infer_variable_types_from_type_maps(self, type_maps: list[TypeMap]) -> dict[SymbolNode, Type]:
    ...
    def remove_capture_conflicts(self, type_map: TypeMap, inferred_types: dict[SymbolNode, Type]) -> None:
        if type_map:
    ...
    def find_isinstance_check(self, node: Expression, *, in_boolean_context: bool = True) -> tuple[TypeMap, TypeMap]:
        ...
        """
        if_map, else_map = self.find_isinstance_check_helper(node, in_boolean_context=in_boolean_context)
    ...
    def find_isinstance_check_helper(
        ...
        if isinstance(node, CallExpr) and len(node.args) != 0:
            ...
            if_condition_map, else_condition_map = self.find_isinstance_check(node.value, in_boolean_context=False)
    ...
    def check_deprecated(self, node: Node | None, context: Context) -> None:
        ...
        if isinstance(node, Decorator):
            node = node.func
        if isinstance(node, (FuncDef, OverloadedFuncDef, TypeInfo)) and (node.deprecated is not None):
    ...
    def warn_deprecated_overload_item(
        ...
        if isinstance(node, OverloadedFuncDef) and isinstance(target, CallableType):
            for item in node.items:
                if isinstance(item, Decorator) and isinstance(candidate := item.func.type, CallableType):
...
def _ambiguous_enum_variants(types: list[Type]) -> set[str]:
    ...
    for t in types:
        ...
        if isinstance(t, UnionType):
            ...
            elif t.type.is_enum and any(base.fullname in ("enum.IntEnum", "enum.StrEnum") for base in t.type.mro):
