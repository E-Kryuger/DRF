    ...
    def check_argument_count(
        ...
        for i, kind in enumerate(callee.arg_kinds):
            if kind.is_required() and not formal_to_actual[i] and not is_unexpected_arg_error:
                ...
                ok = False
            elif callee.param_spec() is not None and not formal_to_actual[i] and callee.special_sig != "partial":
                self.msg.too_few_arguments(callee, context, actual_names)
                ok = False
        return ok
    ...
    def check_op_reversible(
        ...
        for name, method, obj, arg in variants:
            ...
            if local_errors.has_new_errors():
                ...
                if isinstance(obj, Instance) and isinstance(defn := obj.type.get_method(name), OverloadedFuncDef):
    ...
    def visit_index_with_type(self, left_type: Type, e: IndexExpr, original_type: ProperType | None = None) -> Type:
        ...
        if isinstance(left_type, UnionType):
            ...
            if left_type.type_object().is_enum:
                ...
                left_type.type_object().type_vars or left_type.type_object().fullname == "builtins.type"
            ):
