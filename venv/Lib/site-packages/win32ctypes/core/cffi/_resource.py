...
def _EnumResourceLanguages(hModule, lpType, lpName, lpEnumFunc, lParam):
    ...
    check_false(
        dlls.kernel32.EnumResourceLanguagesW(PVOID(hModule), RESOURCE(lpType), RESOURCE(lpName), callback, lParam),
...
def _FindResourceEx(hModule, lpType, lpName, wLanguage):
    return check_null(
        dlls.kernel32.FindResourceExW(PVOID(hModule), RESOURCE(lpType), RESOURCE(lpName), wLanguage),
