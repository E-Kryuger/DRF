...
class SitePackages:
    ...
    def make_candidates(self, path: Path, writable_only: bool = False, strict: bool = False) -> list[Path]:
        ...
        if not results and strict:
            raise RuntimeError(
                f'Unable to find a suitable destination for "{path}" in' f" {paths_csv(self._candidates)}"
            )
    ...
    def write_text(self, path: Path, *args: Any, **kwargs: Any) -> Path:
        paths: tuple[Path, Any] = self._path_method_wrapper(path, "write_text", *args, **kwargs)
        return paths[0]

    def mkdir(self, path: Path, *args: Any, **kwargs: Any) -> Path:
        paths: tuple[Path, Any] = self._path_method_wrapper(path, "mkdir", *args, **kwargs)
        return paths[0]
    ...
    def find(
        ...
        return [
            value[0]
            for value in self._path_method_wrapper(path, "exists", return_first=False, writable_only=writable_only)
            if value[-1] is True
        ]
