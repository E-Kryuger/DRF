...
class Env(ABC):
    ...
    def _find_python_executable(self) -> None:
        ...
        python_executables = sorted(
            p.name for p in bin_dir.glob("python*") if re.match(r"python(?:\d+(?:\.\d+)?)?(?:\.exe)?$", p.name)
        )
    ...
    def _find_pip_executable(self) -> None:
        pip_executables = sorted(
            p.name for p in self._bin_dir.glob("pip*") if re.match(r"pip(?:\d+(?:\.\d+)?)?(?:\.exe)?$", p.name)
        )
        if pip_executables:
    ...
    def scheme_dict(self) -> dict[str, str]:
        ...
        if not self.is_venv() and paths.get("userbase") and ("scripts" in paths and "purelib" in paths):
    ...
    def _run(self, cmd: list[str], **kwargs: Any) -> str:
        ...
        try:
            if call:
                ...
                output = subprocess.check_output(cmd, stderr=stderr, env=env, text=True, encoding=encoding, **kwargs)
