    ...
    def _make_bin_paths(cls, base: Path | None = None) -> list[Path]:
        ...
        if WINDOWS and not sysconfig.get_platform().startswith("mingw"):
            ...
            if base:
                ...
                return [base]
            return [
                *(pi if (pi := p / "install").exists() else p for p in Path.glob(install_dir, "*") if p.is_dir()),
            ]
        return list(Path.glob(install_dir, "**/bin"))

    @classmethod
    def installation_bin_paths(cls, version: Version, implementation: str) -> list[Path]:
