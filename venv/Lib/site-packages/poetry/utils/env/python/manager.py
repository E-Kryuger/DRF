...
class Python:
    ...
    def __init__(
        ...
        if python and (executable or version):
            raise ValueError("When python is provided, neither executable or version must be specified")
    ...
    def find_all(cls) -> Iterator[Python]:
        venv_path: Path | None = Path(os.environ["VIRTUAL_ENV"]) if "VIRTUAL_ENV" in os.environ else None
    ...
    def find_downloadable_versions(
        ...
        *,
        ...
        for pv in PYTHON_VERSIONS:
            for _ in {
                k[1] for k in PYTHON_VERSIONS[pv] if include_incompatible or (k[0], k[1]) == (THIS_PLATFORM, THIS_ARCH)
            }:
                if not constraint.allows(Version.from_parts(pv.major, pv.minor, pv.micro)):
                    continue
    ...
    def get_system_python(cls) -> Python:
        ...
        return cls(
            python=findpython.PythonVersion(
                ...
                _version=packaging.version.Version(".".join(str(v) for v in sys.version_info[:3])),
            )
        )
    ...
    def get_preferred_python(cls, config: Config, io: IO | None = None) -> Python:
        ...
        if not config.get("virtualenvs.use-poetry-python") and (active_python := Python.get_active_python()):
            io.write_error_line(f"Found: {active_python.executable}", verbosity=Verbosity.VERBOSE)
    ...
    def get_compatible_python(cls, poetry: Poetry, io: IO | None = None) -> Python:
        ...
        for python in cls.find_all():
            if python.version.allows_any(supported_python):
                io.write_error_line(f"Using <c1>{python.name}</c1> ({python.patch_version})")
                return python
