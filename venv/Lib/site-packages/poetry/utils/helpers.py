...
class Downloader:
    ...
    def _get(self, start: int = 0) -> Response:
        ...
        response = self._session.get(self._url, stream=True, headers=headers, timeout=REQUESTS_TIMEOUT)
        try:
    ...
    def _iter_content_with_resume(self, chunk_size: int) -> Iterator[bytes]:
        ...
        while True:
            try:
                ...
                if retries < self._max_retries and self.accepts_ranges and fetched_size > 0:
...
def ensure_path(path: str | Path, is_directory: bool = False) -> Path:
    ...
    raise ValueError(f"Specified path '{path}' is not a valid {'directory' if is_directory else 'file'}.")
...
def get_highest_priority_hash_type(hash_types: Collection[str], archive_name: str) -> str | None:
    if not hash_types:
        return None
