...
class Locker:
    ...
    def locked_packages(self) -> dict[Package, TransitivePackageInfo]:
        if not self.is_locked_groups_and_markers():
            raise RuntimeError("This method should not be called if the lock file" " is not at least version 2.1.")
        ...
        for info in locked_package_info:
            ...
            if isinstance(locked_marker, str):
                ...
                markers = {group: parse_marker(locked_marker.get(group, "*")) for group in groups}
    ...
    def _compute_lock_data(self, root: Package, packages: dict[Package, TransitivePackageInfo]) -> TOMLDocument:
    ...
    def _lock_packages(self, packages: dict[Package, TransitivePackageInfo]) -> list[dict[str, Any]]:
        locked = []
    ...
    def _dump_package(self, package: Package, transitive_info: TransitivePackageInfo) -> dict[str, Any]:
