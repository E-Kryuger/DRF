...
class BaseMarker(ABC):
    ...
    @abstractmethod
    def reduce_by_python_constraint(self, python_constraint: VersionConstraint) -> BaseMarker:
        raise NotImplementedError
...
class SingleMarkerLike(BaseMarker, ABC, Generic[SingleMarkerConstraint]):
    def __init__(self, name: str, constraint: SingleMarkerConstraint) -> None:
        ...
        if isinstance(constraint, VersionConstraint):
            self._parser = functools.partial(parse_marker_version_constraint, pep440=name != "platform_release")
        elif name == "extra":
...
class SingleMarker(SingleMarkerLike[Union[BaseConstraint, VersionConstraint]]):
    _CONSTRAINT_RE_PATTERN_1 = re.compile(r"(?i)^(?P<op>~=|!=|>=?|<=?|==?=?|not in|in)?\s*(?P<value>.+)$")
    ...
    def __init__(
        ...
        if m is None:
            raise InvalidMarkerError(f"Invalid marker for '{name}': {constraint_string}")
        ...
        if swapped_name_value and name not in PYTHON_VERSION_MARKERS:
            ...
            parser = functools.partial(parse_marker_version_constraint, pep440=name != "platform_release")
        ...
        try:
            ...
            raise InvalidMarkerError(f"Invalid marker for '{name}': {original_constraint_string}") from e
    ...
    def reduce_by_python_constraint(self, python_constraint: VersionConstraint) -> BaseMarker:
        if self.name in PYTHON_VERSION_MARKERS:
            ...
            python_marker = parse_marker(create_nested_marker("python_version", python_constraint))
...
class AtomicMultiMarker(SingleMarkerLike[MultiConstraint]):
    def __init__(self, name: str, constraint: MultiConstraint) -> None:
        assert all(c.operator in ({"==", "!="} if name == "extra" else {"!="}) for c in constraint.constraints)
    ...
    def __str__(self) -> str:
        return " and ".join(f'{self._name} {c.operator} "{c.value}"' for c in self._constraint.constraints)


class AtomicMarkerUnion(SingleMarkerLike[UnionConstraint]):
    def __init__(self, name: str, constraint: UnionConstraint) -> None:
        assert all(
            isinstance(c, Constraint) and c.operator in ({"==", "!="} if name == "extra" else {"=="})
...
class MultiMarker(BaseMarker):
    ...
    def reduce_by_python_constraint(self, python_constraint: VersionConstraint) -> BaseMarker:
        return self.of(*(m.reduce_by_python_constraint(python_constraint) for m in self._markers))
...
class MarkerUnion(BaseMarker):
                ...
                for i, mark in enumerate(new_markers):
                    ...
                    if not is_one_multi and isinstance(mark, SingleMarkerLike):
                        ...
                        if isinstance(new_marker, SingleMarkerLike) or (
                            isinstance(new_marker, MultiMarker) and new_marker.complexity <= (2, 2)
                        ):
                            new_markers[i] = new_marker
                            included = True
                            break
    ...
    def reduce_by_python_constraint(self, python_constraint: VersionConstraint) -> BaseMarker:
        ...
        if isinstance(python_constraint, (VersionRange, VersionUnion)):
            ...
            if get_python_constraint_from_marker(self.of(*python_only_markers)).allows_all(python_constraint):
                return AnyMarker()

        return self.of(*(m.reduce_by_python_constraint(python_constraint) for m in markers))
...
def _compact_markers(tree_elements: Tree, tree_prefix: str = "", top_level: bool = True) -> BaseMarker:
    ...
    # Combine the groups.
    sub_markers = [group[0] if len(group) == 1 else MultiMarker(*group) for group in groups]
...
def cnf(marker: BaseMarker) -> BaseMarker:
    ...
    if isinstance(marker, MarkerUnion):
        ...
        return MultiMarker.of(*[MarkerUnion.of(*c) for c in _unique_product(*sub_marker_lists)])
...
def dnf(marker: BaseMarker) -> BaseMarker:
    ...
    if isinstance(marker, MultiMarker):
        ...
        return MarkerUnion.of(*[MultiMarker.of(*c) for c in _unique_product(*sub_marker_lists)])
...
def _merge_single_markers(
    ...
    if result_constraint.is_empty():
        ...
        isinstance(c, Constraint) and c.operator in ({"==", "!="} if marker1.name == "extra" else {"=="})
        ...
        c.operator in ({"==", "!="} if marker1.name == "extra" else {"!="}) for c in result_constraint.constraints
    ):
        ...
        if isinstance(result_constraint, VersionRange) and merge_class == MultiMarker:
            ...
            if result_marker is None:
                ...
                result_constraint = get_python_constraint_from_marker(marker1).intersect(
    ...
    if merged_marker and isinstance(merged_marker, SingleMarker):
        ...
        if precision < target_precision:
            if merged_marker.operator in {"<", ">="}:
                target_precision = 2
                marker_string = marker_string.replace("python_full_version", "python_version")
            marker_string = marker_string[:-1] + ".0" * (target_precision - precision) + '"'
        elif (
