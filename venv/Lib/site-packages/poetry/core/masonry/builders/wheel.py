...
class WheelBuilder(Builder):
    ...
    def build(
        ...
        with (
            ...
            zipfile.ZipFile(fd_file, mode="w", compression=zipfile.ZIP_DEFLATED) as zip_file,
        ):
            if self._editable:
...
""",
            ...
            plat_specifier = "-".join((sysconfig.get_platform(), sys.implementation.cache_tag))
...
""",
    ...
    def _zipfile_date_time(self) -> ZipInfoTimestamp:
        ...
        try:
            ...
            logger.warning(
                "SOURCE_DATE_EPOCH environment variable value" " is not an int, setting zipinfo date to default=%s",
                default,
            )
            return default
        except KeyError:
            logger.debug(
                "SOURCE_DATE_EPOCH environment variable not set," " setting zipinfo date to default=%s",
                default,
            )
            return default
        else:
