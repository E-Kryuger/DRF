...
class Builder:
    ...
    def _module(self) -> Module:
        ...
        packages = [item for item in self._package.packages if not self.format or self.format in item["format"]]
        includes = [item for item in self._package.include if not self.format or self.format in item["format"]]
    ...
    def _apply_local_version_label(self) -> None:
        ...
        if local_version_label := self._config_settings.get("local-version"):
            self._poetry.package.version = self._poetry.package.version.replace(local=local_version_label)
                ...
                if file.is_dir():
                    if self.format in formats:
                        for current_file in file.glob("**/*"):
                            ...
                            if not (
                                current_file.is_dir() or self.is_excluded(include_file.relative_to_project_root())
                            ):
    ...
    def get_metadata_content(self) -> str:
        ...
        if self._meta.license:
            ...
            escaped_license = textwrap.indent(self._meta.license, " " * len(license_field), lambda line: True).strip()
    ...
    def convert_entry_points(self) -> dict[str, list[str]]:
        ...
        for group_name, group in self._poetry.package.entry_points.items():
            ...
            result[group_name] = sorted(f"{name} = {specification}" for name, specification in group.items())
