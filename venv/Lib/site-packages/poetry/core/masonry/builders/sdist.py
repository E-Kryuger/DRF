...
class SdistBuilder(Builder):
    ...
    def add_file_to_tar(self, tar: tarfile.TarFile, file_name: str, content: bytes) -> None:
            ...
            if isinstance(include, PackageInclude):
                if include.is_package():
                    ...
                    if pkg_dir is not None:
                        ...
                        if "" in package_dir:
                            package_dir.update(
                                (p, (Path(pkg_root) / p.replace(".", "/")).as_posix()) for p in _packages
                            )
                        else:
    ...
    def _archive_mtime(self) -> int:
        if source_date_epoch := os.getenv("SOURCE_DATE_EPOCH"):
            try:
                ...
                logger.warning("SOURCE_DATE_EPOCH environment variable is not an int," " using mtime=0")
                return 0
