...
def _parse_constraint(constraints: str, constraint_type: type[Constraint]) -> BaseConstraint:
    if constraints == "*":
    ...
    for constraints in or_constraints:
        ...
        if len(and_constraints) > 1:
            for constraint in and_constraints:
                constraint_objects.append(_parse_single_constraint(constraint, constraint_type))
        else:
            constraint_objects.append(_parse_single_constraint(and_constraints[0], constraint_type))
...
def _parse_single_constraint(constraint: str, constraint_type: type[Constraint]) -> Constraint:
    # string comparator
