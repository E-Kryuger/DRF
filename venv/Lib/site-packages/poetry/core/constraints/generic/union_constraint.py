...
class UnionConstraint(BaseConstraint):
    ...
    def allows_any(self, other: BaseConstraint) -> bool:
        if isinstance(other, UnionConstraint):
            return any(c1.allows_any(c2) for c1 in self._constraints for c2 in other.constraints)

        return any(c.allows_any(other) for c in self._constraints)

    def allows_all(self, other: BaseConstraint) -> bool:
        if isinstance(other, UnionConstraint):
            return all(any(c1.allows_all(c2) for c1 in self._constraints) for c2 in other.constraints)
    ...
    def intersect(self, other: BaseConstraint) -> BaseConstraint:
        ...
        if isinstance(other, UnionConstraint) and set(other.constraints) == set(self._constraints):
            return self
