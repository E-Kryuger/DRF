...
class MultiConstraint(BaseConstraint):
    ...
    def allows_any(self, other: BaseConstraint) -> bool:
        ...
        if isinstance(other, UnionConstraint):
            return any(all(c1.allows_any(c2) for c1 in self.constraints) for c2 in other.constraints)
...
class ExtraMultiConstraint(MultiConstraint):
    ...
    def intersect(self, other: BaseConstraint) -> BaseConstraint:
        if isinstance(other, MultiConstraint):
            ...
            for op in self.OPERATORS:
                op_values[op] = {
                    c.value for c in itertools.chain(self._constraints, other.constraints) if c.operator == op
                }
    ...
    def union(self, other: BaseConstraint) -> BaseConstraint:
        ...
        if isinstance(other, Constraint):
            ...
            if len(self._constraints) == 2 and other.value in (c.value for c in self._constraints):
