...
class Constraint(BaseConstraint):
    ...
    def allows(self, other: BaseConstraint) -> bool:
        if not isinstance(other, Constraint) or other.operator != "==":
            raise ValueError(
                f"Invalid argument for allows" f' ("other" must be a constraint with operator "=="): {other}'
            )
    ...
    def allows_any(self, other: BaseConstraint) -> bool:
        ...
        if isinstance(other, Constraint):
            ...
            return self._operator == "!=" and any(self.allows_any(c) for c in other.constraints)
    ...
    def union(self, other: BaseConstraint) -> BaseConstraint:
        ...
        if isinstance(other, Constraint):
            ...
            if (
                (ops in ({"!="}, {"not in"}))
                or (
                    (ops in ({"in", "!="}, {"in", "not in"}) and (self.operator == "in" and self.value in other.value))
...
class ExtraConstraint(Constraint):
    def __init__(self, value: str, operator: str = "==") -> None:
        ...
        if self._operator not in {"==", "!="}:
            raise ValueError('Only the operators "==" and "!=" are supported for extra constraints')
