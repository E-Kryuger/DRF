...
class DependencyGroup:
    def __init__(self, name: str, *, optional: bool = False, mixed_dynamic: bool = False) -> None:
        self._name: str = name
...
def _enrich_dependency(
    ...
    if (
        ...
        raise ValueError(
            "Cannot enrich dependency with different sources: " f"{project_dependency} and {poetry_dependency}"
        )

    constraint = project_dependency.constraint.intersect(poetry_dependency.constraint)
    if constraint.is_empty():
        raise ValueError(
            "Cannot enrich dependency with incompatible constraints: " f"{project_dependency} and {poetry_dependency}"
        )
