...
class Factory:
    ...
    def create_poetry(self, cwd: Path | None = None, with_groups: bool = True) -> Poetry:
        ...
        self.configure_package(package, pyproject, poetry_file.parent, with_groups=with_groups)
    ...
    def configure_package(
        ...
        cls._configure_package_dependencies(package, project, tool_poetry, with_groups=with_groups)
    ...
    def _configure_package_metadata(
        ...
        for key in ("authors", "maintainers"):
            if entries := project.get(key):
                ...
                participants = [combine_unicode(author) for author in tool_poetry.get(key, [])]
            if key == "authors":
                package.authors = participants
            else:
                package.maintainers = participants

        package.description = project.get("description") or tool_poetry.get("description", "")
        if project_license := project.get("license"):
            if isinstance(project_license, str):
                ...
                if not raw_license and (license_file := cast("str", project_license.get("file", ""))):
                    license_path = (root / license_file).absolute()
                    try:
                        raw_license = Path(license_path).read_text(encoding="utf-8")
                    except FileNotFoundError as e:
                        raise FileNotFoundError(f"Poetry: license file '{license_path}' not found") from e
        else:
        ...
        package.classifiers = (static_classifiers := project.get("classifiers")) or tool_poetry.get("classifiers", [])
        ...
        if readme := project.get("readme"):
            ...
            custom_readmes = (custom_readme,) if isinstance(custom_readme, str) else custom_readme
        ...
        if other_scripts := project.get("entry-points"):
            for group_name, scripts in sorted(other_scripts.items()):
                if group_name in {"console-scripts", "gui-scripts"}:
                    raise ValueError(
                        f"Group '{group_name}' is reserved and cannot be used" " as a custom entry-point group."
                    )
    ...
    def _configure_package_dependencies(
        ...
        if dependencies or optional_dependencies:
            group = DependencyGroup(
                MAIN_GROUP,
                mixed_dynamic=("dependencies" in dynamic or "optional-dependencies" in dynamic),
            )
            package.add_dependency_group(group)

            for constraint in dependencies:
                group.add_dependency(Dependency.create_from_pep_508(constraint, relative_to=package.root_dir))
            package_extras = {}
            for extra_name, dependencies in optional_dependencies.items():
                ...
                for dependency_constraint in dependencies:
                    dependency = Dependency.create_from_pep_508(dependency_constraint, relative_to=package.root_dir)
    ...
    @classmethod
    def _configure_package_poetry_specifics(cls, package: ProjectPackage, tool_poetry: dict[str, Any]) -> None:
        ...
        if includes := tool_poetry.get("include"):
            includes = [include if isinstance(include, dict) else {"path": include} for include in includes]
        ...
        if packages := tool_poetry.get("packages"):
            package.packages = cls._prepare_formats(packages, default_formats=["sdist", "wheel"])
    ...
    @classmethod
    def validate(cls, toml_data: dict[str, Any], strict: bool = False) -> dict[str, list[str]]:
        """
        ...
        if project is not None:
            project_validation_errors = [
                e.replace("data", "project") for e in validate_object(project, "project-schema")
            ]
        ...
        tool_poetry_validation_errors = [
            e.replace("data.", "tool.poetry.") for e in validate_object(tool_poetry, "poetry-schema")
        ]
            ...
            for key in ("name", "version"):
                ...
                if not value:
                    result["errors"].append(
                        f"Either [project.{key}] or [tool.poetry.{key}]" " is required in package mode."
                    )
    ...
    @classmethod
    def _validate_legacy_vs_project(cls, toml_data: dict[str, Any], result: dict[str, list[str]]) -> None:
        project = toml_data.get("project", {})
        dynamic = project.get("dynamic", [])
        ...
        for name, deprecated, new_name in redundant_fields:
            ...
            if name in tool_poetry:
                ...
                if new_name in project:
                    warning = (
                        f"[project.{new_name}] and [tool.poetry.{name}] are both set." " The latter will be ignored."
                    )
                elif deprecated:
                    warning = f"[tool.poetry.{name}] is deprecated." f" Use [project.{new_name}] instead."
    ...
    @classmethod
    def _validate_strict(cls, config: dict[str, Any], result: dict[str, list[str]]) -> None:
