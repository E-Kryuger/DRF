...
class LazyWheelOverHTTP(LazyFileOverHTTP):
    ...
    def _parse_full_length_from_content_range(arg: str) -> int:
        ...
        if m is None:
            raise HTTPRangeRequestUnsupportedError(f"could not parse Content-Range: '{arg}'")
        ...
        for info in zf.infolist():
            if start is None:
                ...
                if not self._metadata_regex.search(info.filename):
                    ...
                    break
        if start is None:
            raise UnsupportedWheelError(f"no {self._metadata_regex!r} found for {name} in {self.name}")
