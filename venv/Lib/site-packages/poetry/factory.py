...
class Factory(BaseFactory):
    ...
    def create_poetry(
        ...
        if version_str := base_poetry.local_config.get("requires-poetry"):
            ...
            if not version_constraint.allows(version):
                raise PoetryError(
                    f"This project requires Poetry {version_constraint}," f" but you are using Poetry {version}"
                )
    ...
    def create_pool(
        ...
        if not explicit_pypi:
            if pool.has_primary_repositories():
                ...
                pool.add_repository(
                    cls.create_package_source({"name": "pypi"}, config, disable_cache=disable_cache),
                    priority=Priority.PRIMARY,
                )

        if not pool.repositories:
            raise PoetryError("At least one source must not be configured as 'explicit'.")
    ...
    def validate(cls, toml_data: dict[str, Any], strict: bool = False) -> dict[str, list[str]]:
        ...
        project_name = toml_data.get("project", {}).get("name") or poetry_config.get("name")
