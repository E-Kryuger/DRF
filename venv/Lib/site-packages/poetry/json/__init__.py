...
def validate_object(obj: dict[str, Any]) -> list[str]:
    schema = json.loads((files(__package__) / "schemas" / "poetry.json").read_text(encoding="utf-8"))
    ...
    core_schema = json.loads(
        (files("poetry.core") / "json" / "schemas" / "poetry-schema.json").read_text(encoding="utf-8")
    )
