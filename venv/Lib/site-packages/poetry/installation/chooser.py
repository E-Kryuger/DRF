    ...
    def choose_for(self, package: Package) -> Link:
        ...
        for link in self._get_links(package):
            ...
            if link.is_sdist and not self._only_binary_policy.allows(package.name):
                logger.debug(
                    "Skipping source distribution for %s as requested in only binary policy for" " package (%s)",
                    link.filename,
                    package.name,
                )
                sdists_skipped += 1
        ...
        if not links:
            raise self._no_links_found_error(package, links_seen, wheels_skipped, sdists_skipped, unsupported_wheels)
    ...
    def _no_links_found_error(
        ...
        if unsupported_wheels:
            messages += [
                ...
                ConsoleMessage("\n".join(unsupported_wheels), debug=True).indent("  - ").wrap("warning"),
                ConsoleMessage(
