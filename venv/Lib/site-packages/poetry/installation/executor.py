...
class Executor:
    def __init__(
        ...
        self._build_config_settings: Mapping[NormalizedName, Mapping[str, str | Sequence[str]]] = config.get(
        ...
        for _, group in groups:
            ...
            for operation in group:
                ...
                if is_parallel_unsafe:
                    ...
                    serial_git_operations[_package_get_name(operation.package)].append(operation)
                else:
            ...
            try:
                ...
                with self._lock:
                    ...
                    if isinstance(e, IsolatedBuildBackendError):
                        ...
                        message = (
                            "<error>" "Cannot install build-system.requires" f" for {pkg.pretty_name}." "</error>"
                        )
                    elif isinstance(e, SolverProblemError):
                        message = (
                            "<error>" "Cannot resolve build-system.requires" f" for {pkg.pretty_name}." "</error>"
                        )
    ...
    def _download_archive(
        ...
        downloader = Downloader(url, dest, self._authenticator, max_retries=self._max_retries)
