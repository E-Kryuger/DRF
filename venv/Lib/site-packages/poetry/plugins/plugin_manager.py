...
class ProjectPluginCache:
    ...
    def _config(self) -> dict[str, Any]:
        return {
            "python": sys.version,
            "poetry": __version__,
            "plugins-hash": hashlib.sha256(json.dumps(self._plugin_section, sort_keys=True).encode()).hexdigest(),
        }
    ...
    def ensure_plugins(self) -> None:
        ...
        if not plugins:
            if self._path.exists():
                self._io.write_line("<info>No project plugins defined." " Removing the project's plugin cache</info>")
                self._io.write_line("")
        ...
        if self._is_fresh():
            ...
            self._io.write_line("Removing the project's plugin cache because it is outdated")
        ...
        relevant_plugins = {plugin.name: plugin for plugin in plugins if plugin.marker.validate(poetry_env.marker_env)}
        if not relevant_plugins:
            if self._io.is_debug():
                self._io.write_line("No relevant project plugins for Poetry's environment defined.")
                self._io.write_line("")
            self._write_config()
            return

        self._io.write_line(
            "<info>Ensuring that the Poetry plugins required" " by the project are available...</info>"
        )
        ...
        if missing_plugin_count == 0 and not insufficient_plugins:
            ...
            self._write_config()
            self._io.write_line("All required plugins have already been installed" " in Poetry's environment.")
            self._io.write_line("")
            return
        ...
        missing_plugins = [plugin for name, plugin in relevant_plugins.items() if name not in satisfied_plugins]
