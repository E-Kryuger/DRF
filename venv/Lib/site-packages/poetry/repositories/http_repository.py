...
class HTTPRepository(CachedRepository):
        ...
        if link.has_metadata:
            try:
                ...
                if link.metadata_hashes and (
                    hash_name := get_highest_priority_hash_type(link.metadata_hashes, f"{link.filename}.metadata")
                ):
    ...
    def _get_info_from_links(self, links: list[Link], *, ignore_yanked: bool) -> PackageInfo:
        ...
        if wheels:
            ...
            if universal_python2_wheel and universal_python3_wheel:
                ...
                if py3_info.requires_dist:
                    ...
                    py3_requires_dist = {
                        Dependency.create_from_pep_508(r).to_pep_508() for r in py3_info.requires_dist
                    }
    ...
    def _links_to_data(self, links: list[Link], data: PackageInfo) -> dict[str, Any]:
        ...
        for link in links:
            ...
            if file_hash is None and (hash_type := get_highest_priority_hash_type(link.hashes, link.filename)):
