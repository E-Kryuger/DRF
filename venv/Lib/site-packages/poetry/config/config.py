...
class Config:
    ...
    @staticmethod
    def _get_environment_build_config_settings() -> Mapping[NormalizedName, Mapping[str, str | Sequence[str]]]:
        ...
        for env_key in os.environ:
            if match := pattern.match(env_key):
                if not build_config_setting_validator(os.environ[env_key]):
                    logger.debug("Invalid value set for environment variable %s", env_key)
                    continue
                build_config_settings[canonicalize_name(match.group("name"))] = build_config_setting_normalizer(
                    os.environ[env_key]
                )
    ...
    def get(self, setting_name: str, default: Any = None) -> Any:
        ...
        build_config_settings: Mapping[NormalizedName, Mapping[str, str | Sequence[str]]] = {}
        ...
        if self._use_environment:
            ...
            if setting_name == build_config_settings_key or setting_name.startswith(f"{build_config_settings_key}."):
        ...
        for package_name in build_config_settings:
            value["installer"]["build-config-settings"][package_name] = build_config_settings[package_name]
