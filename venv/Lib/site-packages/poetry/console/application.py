...
COMMAND_NOT_FOUND_PREFIX_MESSAGE = "Looks like you're trying to use a Poetry command that is not available."
...
class Application(BaseApplication):
        ...
        with directory(self._working_directory):
            try:
                ...
                if command is not None:
                    ...
                    io.write_error_line(f"The requested command <c1>{command}</> does not exist.")
    ...
    def _error_write_command_suggestions(self, io: IO, suggested_names: list[str], doc_tag: str | None = None) -> None:
        if suggested_names:
            suggestion_lines = [
    ...
    def _configure_global_options(self, io: IO) -> None:
        ...
        self._working_directory = ensure_path(io.input.option("directory") or Path.cwd(), is_directory=True)
        ...
        if self._project_directory is not None:
            ...
            self._project_directory = ensure_path(
                (
                    ...
                    else self._working_directory.joinpath(self._project_directory).resolve(strict=False)
                ),
                is_directory=True,
            )
    ...
    def _configure_run_command(self, io: IO) -> None:
        ...
        if command_name == "run":
            ...
            without_command = ArgvInput([self._name or "", *tokens_without_command], None)
            ...
            subcommand_index = (tokens_without_command.index(subcommand) if subcommand else 0) + command_index + 1
