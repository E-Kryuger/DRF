...
class PrettyCalledProcessError:
    ...
    def __post_init__(self, exception: CalledProcessError, indent: str) -> None:
        ...
        self.command = shlex.join(exception.cmd) if isinstance(exception.cmd, list) else exception.cmd
...
class PoetryRuntimeError(PoetryConsoleError):
    ...
    def create(
        ...
        if isinstance(exception, CalledProcessError):
            ...
            messages.insert(
                0,
                ConsoleMessage(str(exception), debug=True).make_section("Exception", indent="    | "),
            )
