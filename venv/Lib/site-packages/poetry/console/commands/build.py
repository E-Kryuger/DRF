...
class BuildHandler:
    ...
    def _requires_isolated_build(self) -> bool:
        ...
        if self.poetry.package.build_script or len(self.poetry.build_system_dependencies) != 1:
            return True
        ...
        return bool(not build_dependency.constraint.allows(poetry_core_version) or build_dependency.source_type)
    ...
    def build(self, options: BuildOptions) -> int:
        if not self.poetry.is_package_mode:
            self.io.write_error_line("Building a package is not possible in non-package mode.")
            return 1

        dist_dir = Path(options.output)
        package = self.poetry.package
        self.io.write_line(f"Building <c1>{package.pretty_name}</c1> (<c2>{package.version}</c2>)")
