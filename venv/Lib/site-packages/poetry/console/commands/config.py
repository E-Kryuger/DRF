...
CONFIG_MIGRATIONS = [
    ConfigSourceMigration(old_key="experimental.system-git-client", new_key="system-git-client"),
...
To remove a repository (repo is a short alias for repositories):
    ...
    def handle(self) -> int:
        ...
        if not self.argument("value") and not self.option("unset"):
            ...
            if m := re.match(r"installer\.build-config-settings(\.([^.]+))?", self.argument("key")):
                if not m.group(1):
                    ...
                    if value := config.get(key):
                        ...
                        self.line(f"No build config settings configured for <c1>{package_name}</>.")
                return 0
    ...
    def _migrate(self) -> None:
        ...
        required_migrations = [
            migration for migration in CONFIG_MIGRATIONS if migration.dry_run(config_source, io=self.io)
        ]
        ...
        if not self.io.is_interactive() or self.confirm("Proceed with migration?: ", False):
