...
class SearchCommand(Command):
    ...
    arguments: ClassVar[list[Argument]] = [argument("tokens", "The tokens to search for.", multiple=True)]

    def handle(self) -> int:
        seen = set()

        table = self.table(style="compact")
        table.set_headers(["<b>Package</>", "<b>Version</>", "<b>Source</>", "<b>Description</>"])
        ...
        for package, source in sorted(rows, key=lambda x: (x[0].name, x[0].version, x[1])):
            table.add_row(
                [
                    f"<c1>{package.name}</>",
