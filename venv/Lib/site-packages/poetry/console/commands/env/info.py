...
class EnvInfoCommand(Command):
    ...
    options: ClassVar[list[Option]] = [
        ...
        option("executable", "e", "Only display the environment's python executable path."),
    ]
    ...
    def _display_complete_info(self, env: Env) -> None:
        ...
        listing = [
            ...
            ("<info>Path</info>:          " f" <comment>{env.path if env.is_venv() else 'NA'}</>"),
            ("<info>Executable</info>:    " f" <comment>{env.python if env.is_venv() else 'NA'}</>"),
        ]
        if env.is_venv():
            listing.append(
                "<info>Valid</info>:         " f" <{'comment' if env.is_sane() else 'error'}>{env.is_sane()}</>"
            )
        self.line("\n".join(listing))
