...
class RemoveCommand(InstallerCommand):
    ...
    arguments: ClassVar[list[Argument]] = [argument("packages", "The packages to remove.", multiple=True)]
    options: ClassVar[list[Option]] = [
        option("group", "G", "The group to remove the dependency from.", flag=False),
        option(
            "dev",
            "D",
            "Remove a package from the development dependencies." " (shortcut for '-G dev')",
        ),
        option(
            "dry-run",
            None,
    ...
    def handle(self) -> int:
        ...
        if group is None:
            ...
            if project_dependencies or poetry_dependencies:
                group_sections.append((MAIN_GROUP, project_dependencies, poetry_dependencies))
            ...
            for group_name, project_section, poetry_section in group_sections:
                removed |= self._remove_packages(packages, project_section, poetry_section, group_name)
