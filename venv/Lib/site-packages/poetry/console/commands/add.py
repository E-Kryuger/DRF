...
class AddCommand(InstallerCommand, InitCommand):
    ...
    arguments: ClassVar[list[Argument]] = [argument("name", "The packages to add.", multiple=True)]
...
"""
    ...
    def handle(self) -> int:
        ...
        project_name = (
            canonicalize_name(name) if (name := project_content.get("name", poetry_content.get("name"))) else None
        )

        use_project_section = False
        project_dependency_names = []
        if group == MAIN_GROUP:
            if "dependencies" in project_content or "optional-dependencies" in project_content:
                use_project_section = True
                if optional:
                    project_section = project_content.get("optional-dependencies", {}).get(optional, array())
                else:
                    project_section = project_content.get("dependencies", array())
                project_dependency_names = [Dependency.create_from_pep_508(dep).name for dep in project_section]
            else:
        ...
        existing_packages = self.get_existing_packages_from_input(packages, poetry_section, project_dependency_names)
    ...
    def _add_dependency_to_extras(self, dependency: Dependency, extra_name: NormalizedName) -> None:
