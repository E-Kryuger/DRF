...
"""
    ...
    def _init_pyproject(
        ...
        if pyproject.file.exists():
            if pyproject.is_poetry_project():
                self.line_error(
                    "<error>A pyproject.toml file with a project and/or" " a poetry section already exists.</error>"
                )
                return 1
        ...
        if not python:
            ...
            python = ">=" + Python.get_preferred_python(config, self.io).minor_version.to_string()
    ...
    def _parse_requirements(self, requirements: list[str]) -> list[dict[str, Any]]:
        ...
        return [parser.parse(re.sub(r"@\s*latest$", "", requirement, flags=re.I)) for requirement in requirements]
