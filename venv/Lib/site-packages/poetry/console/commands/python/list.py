...
class PythonListCommand(Command):
    ...
    arguments: ClassVar[list[Argument]] = [argument("version", "Python version to search for.", optional=True)]
    ...
    options: ClassVar[list[Option]] = [
        ...
        option("implementation", "i", "Python implementation to search for.", flag=False),
        option("managed", "m", "List only Poetry managed Python versions.", flag=True),
    ]

    description = "Shows Python versions available for this environment." " (<warning>experimental feature</warning>)"
    ...
    def handle(self) -> int:
        ...
        if self.argument("version"):
            ...
            try:
                ...
                self.io.write_error_line(f"<error>Invalid Python version requested <b>{request}</></error>")
                return 1

        for info in Python.find_all_versions(constraint=constraint, implementation=self.option("implementation")):
            rows.append(info)
        ...
        rows.sort(key=lambda x: (x.major, x.minor, x.patch, x.implementation), reverse=True)
        ...
        for pv in rows:
            ...
            implementation = implementations.get(pv.implementation.lower(), pv.implementation)
            is_poetry_managed = pv.executable is None or pv.executable.resolve().is_relative_to(
            ...
            manager = "<fg=blue>Poetry</>" if is_poetry_managed else "<fg=yellow>System</>"
            path = f"<fg=green>{pv.executable.as_posix()}</>" if pv.executable else "Available for download"
