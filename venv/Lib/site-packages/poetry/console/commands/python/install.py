...
class PythonInstallCommand(Command):
    ...
    arguments: ClassVar[list[Argument]] = [argument("python", "The python version to install.")]

    options: ClassVar[list[Option]] = [
        option("clean", "c", "Clean up installation if check fails.", flag=True),
        option("free-threaded", "t", "Use free-threaded version if available.", flag=True),
        option(
            "implementation",
            "i",
        ...
        option("reinstall", "r", "Reinstall if installation already exists.", flag=True),
    ]
    ...
    def handle(self) -> int:
        ...
        try:
            ...
            self.io.write_error_line("No suitable standalone build found for the requested Python version.")
            return 1
        ...
        try:
            ...
            if installer.installation_directory.exists() and self.option("clean"):
                PythonRemoveCommand.remove_python_installation(str(installer.version), impl, self.io)
