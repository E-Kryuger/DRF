...
class PythonRemoveCommand(Command):
    ...
    arguments: ClassVar[list[Argument]] = [argument("python", "The python version to remove.", multiple=True)]
    ...
    description = (
        "Remove the specified Python version if managed by Poetry." " (<warning>experimental feature</warning>)"
    )
    ...
    def remove_python_installation(request: str, implementation: str, io: IO) -> int:
        try:
            ...
            io.write_error_line(f"<error>Invalid Python version requested <b>{request}</></error>")
            return 1
