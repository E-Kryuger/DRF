    ...
    def calculate_operations(
        ...
        *,
        ...
        if isinstance(self._result_packages, dict):
            priorities = {pkg: info.depth for pkg, info in self._result_packages.items()}
        else:
        ...
        for result_package in self._result_packages:
            ...
            if self._marker_env:
                ...
                if info.groups & self._groups and info.get_marker(self._groups).validate(marker_env_with_extras):
        ...
        if with_uninstalls:
            ...
            for current_package in self._current_packages:
                if current_package.name not in (result_packages | uninstalls) and (
                    installed_package := self._installed_packages.get(current_package.name)
                ):
