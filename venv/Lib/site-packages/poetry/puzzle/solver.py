...
class Solver:
        ...
        with self._progress(), self._provider.use_latest_for(use_latest or []):
            ...
            for transitive_info in packages.values():
                for group, marker in transitive_info.markers.items():
                    transitive_info.markers[group] = simplify_marker(marker, self._package.python_constraint)
            end = time.time()
    ...
    def _solve_in_compatibility_mode(
        ...
        return merge_override_packages(override_packages, self._package.python_constraint)
    ...
    def _aggregate_solved_packages(self, packages: list[Package]) -> dict[Package, TransitivePackageInfo]:
        combined_nodes, markers = depth_first_search(PackageNode(self._package, packages))
                ...
                for _package in packages:
                    if not _package.features and _package.name == package.name and _package.version == package.version:
                        for dep in package.requires:
                            ...
                            # Avoid duplication.
                            if any(_dep == dep and _dep.marker == dep.marker for _dep in _package.requires):
                                continue
...
def dfs_visit(
    ...
    for out_neighbor in node.reachable():
        ...
        markers[out_neighbor.package][node.package] = marker.union(
            out_neighbor.marker if node.package.is_root() else out_neighbor.marker.without_extras()
        )
    ...
    def reachable(self) -> Sequence[PackageNode]:
        ...
        for dependency in self.package.all_requires:
            for pkg in self.packages:
                if pkg.complete_name == dependency.complete_name and pkg.satisfies(dependency):
                    marker = dependency.marker
                    if self.package.is_root() and dependency.in_extras:
                        marker = marker.intersect(
                            parse_marker(" or ".join(f'extra == "{extra}"' for extra in dependency.in_extras))
                        )
                    children.append(
                        PackageNode(
                            pkg,
...
def calculate_markers(packages: dict[Package, TransitivePackageInfo], markers: MarkerOriginDict) -> None:
    ...
    while has_incomplete_markers:
        ...
        for depth in range(max_depth + 1):
            for package in packages_by_depth[depth]:
                ...
                transitive_marker: dict[str, BaseMarker] = {group: EmptyMarker() for group in transitive_info.groups}
...
def merge_override_packages(
    override_packages: list[tuple[dict[Package, dict[str, Dependency]], dict[Package, TransitivePackageInfo]]],
    python_constraint: VersionConstraint,
) -> dict[Package, TransitivePackageInfo]:
    result: dict[Package, TransitivePackageInfo] = {}
    all_packages: dict[Package, list[tuple[Package, TransitivePackageInfo, BaseMarker]]] = {}
    for override, o_packages in override_packages:
        ...
        for package, info in o_packages.items():
            ...
            all_packages.setdefault(package, []).append((package, info, override_marker))
    for package_duplicates in all_packages.values():
        ...
        package_info.groups = {g for _, info, _ in package_duplicates for g in info.groups}
        if all(info.markers == package_info.markers for _, info, _ in remaining):
            ...
            package_info.markers = {
                group: override_marker.intersect(marker) for group, marker in package_info.markers.items()
            }
        else:
            ...
            for _, info, override_marker in remaining:
                for group, marker in info.markers.items():
                    package_info.markers[group] = package_info.markers.get(group, EmptyMarker()).union(
                        override_marker.intersect(marker)
                    )
...
@functools.cache
def simplify_marker(marker: BaseMarker, python_constraint: VersionConstraint) -> BaseMarker:
    """
