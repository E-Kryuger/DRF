...
class Provider:
    ...
    def use_environment(self, env: Env) -> Iterator[Provider]:
        ...
        self._env = env
        self._package_python_constraint = Version.parse(env.marker_env["python_full_version"])
                ...
                if (
                    ...
                    return [
                        Incompatibility(
                            ...
                            PythonCauseError(package.python_versions, str(self._python_constraint)),
                        )
                    ]
    ...
    def complete_package(self, dependency_package: DependencyPackage) -> DependencyPackage:
        ...
        for dep in requires:
            ...
            if self._env:
                marker_values = (
                    self._marker_values(self._active_root_extras) if package.is_root() else self._env.marker_env
                )
            ...
            if self._env is None and package.is_root() and dep.in_extras:
                ...
                dep.marker = dep.marker.intersect(
                    parse_marker(" or ".join(f'extra == "{extra}"' for extra in dep.in_extras))
                )
        ...
        for dep_name, deps in duplicates.items():
            ...
            if len(duplicates_by_extras) == 1:
                ...
                for complete_dep_name, deps_by_extra in duplicates_by_extras.items():
                    if len(deps_by_extra) > 1:
                        duplicates_by_extras[complete_dep_name] = self._resolve_overlapping_markers(
                ...
                if all(len(d) == 1 for d in duplicates_by_extras.values()) and all(
                    ...
                    for d1, d2 in itertools.combinations(duplicates_by_extras.values(), 2)
                ):
            ...
            deps = [dep for dep in deps if not self._overrides_marker_intersection.intersect(dep.marker).is_empty()]
            if len(deps) < 2:
